...
TABS.pid_tuning.initialize = function (callback) {
    ...
    MSP.promise(MSPCodes.MSP_STATUS)
        .then(function () {
            if (
                semver.gte(
                    CONFIG.apiVersion,
                    CONFIGURATOR.API_VERSION_MIN_SUPPORTED_PID_CONTROLLER_CHANGE
                )
            ) {
    ...
    function pid_and_rc_to_form() {
        ...
        $('.pid_tuning input[name="rc_rate"]').val(
            RC_tuning.RC_RATE.toFixed(2)
        );
        $('.pid_tuning input[name="roll_pitch_rate"]').val(
            RC_tuning.roll_pitch_rate.toFixed(2)
        );
        $('.pid_tuning input[name="roll_rate"]').val(
            RC_tuning.roll_rate.toFixed(2)
        );
        $('.pid_tuning input[name="pitch_rate"]').val(
            RC_tuning.pitch_rate.toFixed(2)
        );
        $('.pid_tuning input[name="yaw_rate"]').val(
            RC_tuning.yaw_rate.toFixed(2)
        );
        $('.pid_tuning input[name="rc_expo"]').val(
            RC_tuning.RC_EXPO.toFixed(2)
        );
        $('.pid_tuning input[name="rc_yaw_expo"]').val(
            RC_tuning.RC_YAW_EXPO.toFixed(2)
        );

        $('.throttle input[name="mid"]').val(RC_tuning.throttle_MID.toFixed(2));
        $('.throttle input[name="expo"]').val(
            RC_tuning.throttle_EXPO.toFixed(2)
        );

        $('.tpa input[name="tpa"]').val(RC_tuning.dynamic_THR_PID.toFixed(2));
        $('.tpa input[name="tpa-breakpoint"]').val(
            RC_tuning.dynamic_THR_breakpoint
        );
        ...
        if (semver.gte(CONFIG.apiVersion, "1.16.0")) {
            $('input[id="vbatpidcompensation"]').prop(
                "checked",
                ADVANCED_TUNING.vbatPidCompensation !== 0
            );
        }
        ...
        if (semver.gte(CONFIG.apiVersion, "1.16.0")) {
            $('.pid_tuning input[name="rc_rate_yaw"]').val(
                RC_tuning.rcYawRate.toFixed(2)
            );
            $('.pid_filter input[name="gyroLowpassFrequency"]').val(
                FILTER_CONFIG.gyro_lowpass_hz
            );
            $('.pid_filter input[name="dtermLowpassFrequency"]').val(
                FILTER_CONFIG.dterm_lowpass_hz
            );
            $('.pid_filter input[name="yawLowpassFrequency"]').val(
                FILTER_CONFIG.yaw_lowpass_hz
            );
        } else {
        ...
        if (
            semver.gte(CONFIG.apiVersion, "1.20.0") ||
            (semver.gte(CONFIG.apiVersion, "1.16.0") &&
                FEATURE_CONFIG.features.isEnabled("SUPEREXPO_RATES"))
        ) {
        ...
        if (semver.gte(CONFIG.apiVersion, "1.20.0")) {
            $('.pid_filter input[name="gyroNotch1Frequency"]').val(
                FILTER_CONFIG.gyro_notch_hz
            );
            $('.pid_filter input[name="gyroNotch1Cutoff"]').val(
                FILTER_CONFIG.gyro_notch_cutoff
            );
            $('.pid_filter input[name="dTermNotchFrequency"]').val(
                FILTER_CONFIG.dterm_notch_hz
            );
            $('.pid_filter input[name="dTermNotchCutoff"]').val(
                FILTER_CONFIG.dterm_notch_cutoff
            );

            var dtermSetpointTransitionNumberElement = $(
                'input[name="dtermSetpointTransition-number"]'
            );
            if (semver.gte(CONFIG.apiVersion, "1.38.0")) {
                dtermSetpointTransitionNumberElement.attr("min", 0.0);
            } else {
                dtermSetpointTransitionNumberElement.attr("min", 0.01);
            }

            dtermSetpointTransitionNumberElement.val(
                ADVANCED_TUNING.dtermSetpointTransition / 100
            );

            $('input[name="dtermSetpoint-number"]').val(
                ADVANCED_TUNING.dtermSetpointWeight / 100
            );
        } else {
        ...
        if (semver.gte(CONFIG.apiVersion, "1.21.0")) {
            $('.pid_filter input[name="gyroNotch2Frequency"]').val(
                FILTER_CONFIG.gyro_notch2_hz
            );
            $('.pid_filter input[name="gyroNotch2Cutoff"]').val(
                FILTER_CONFIG.gyro_notch2_cutoff
            );
        } else {
        ...
        if (semver.gte(CONFIG.apiVersion, "1.24.0")) {
            $('.pid_tuning input[name="angleLimit"]').val(
                ADVANCED_TUNING.levelAngleLimit
            );
            $('.pid_tuning input[name="sensitivity"]').val(
                ADVANCED_TUNING.levelSensitivity
            );
        } else {
        ...
        if (semver.gte(CONFIG.apiVersion, "1.36.0")) {
            $('.pid_filter select[name="dtermLowpassType"]').val(
                FILTER_CONFIG.dterm_lowpass_type
            );
            $('.antigravity input[name="itermThrottleThreshold"]').val(
                ADVANCED_TUNING.itermThrottleThreshold
            );
            $('.antigravity input[name="itermAcceleratorGain"]').val(
                ADVANCED_TUNING.itermAcceleratorGain / 1000
            );

            var antiGravitySwitch = $("#antiGravitySwitch");
            antiGravitySwitch.prop(
                "checked",
                ADVANCED_TUNING.itermAcceleratorGain !== 1000
            );
            antiGravitySwitch.change(function () {
                ...
                if (checked) {
                    if (ADVANCED_TUNING.itermAcceleratorGain === 1000) {
                        const DEFAULT_ACCELERATOR_GAIN = semver.gte(
                            CONFIG.apiVersion,
                            API_VERSION_1_43
                        )
                            ? 3.5
                            : 1.1;
                        $(
                            '.antigravity input[name="itermAcceleratorGain"]'
                        ).val(DEFAULT_ACCELERATOR_GAIN);
                    } else {
                        const itermAcceleratorGain =
                            ADVANCED_TUNING.itermAcceleratorGain / 1000;
                        $(
                            '.antigravity input[name="itermAcceleratorGain"]'
                        ).val(itermAcceleratorGain);
                    }
        ...
        if (semver.gte(CONFIG.apiVersion, "1.37.0")) {
            $('.pid_tuning input[name="rc_rate_pitch"]').val(
                RC_tuning.rcPitchRate.toFixed(2)
            );
            $('.pid_tuning input[name="rc_pitch_expo"]').val(
                RC_tuning.RC_PITCH_EXPO.toFixed(2)
            );
        }

        if (semver.gte(CONFIG.apiVersion, "1.39.0")) {
            $('.pid_filter input[name="gyroLowpass2Frequency"]').val(
                FILTER_CONFIG.gyro_lowpass2_hz
            );
            $('.pid_filter select[name="gyroLowpassType"]').val(
                FILTER_CONFIG.gyro_lowpass_type
            );
            $('.pid_filter select[name="gyroLowpass2Type"]').val(
                FILTER_CONFIG.gyro_lowpass2_type
            );
            $('.pid_filter input[name="dtermLowpass2Frequency"]').val(
                FILTER_CONFIG.dterm_lowpass2_hz
            );

            // We load it again because the limits are now bigger than in 1.16.0
            $('.pid_filter input[name="gyroLowpassFrequency"]').attr(
                "max",
                "16000"
            );
            $('.pid_filter input[name="gyroLowpassFrequency"]').val(
        ...
        if (semver.gte(CONFIG.apiVersion, "1.40.0")) {
            // I Term Rotation
            $('input[id="itermrotation"]').prop(
                "checked",
                ADVANCED_TUNING.itermRotation !== 0
            );

            // Smart Feed Forward
            $('input[id="smartfeedforward"]').prop(
                "checked",
                ADVANCED_TUNING.smartFeedforward !== 0
            );
            ...
            $('select[id="itermrelaxAxes"]').val(
                ADVANCED_TUNING.itermRelax > 0 ? ADVANCED_TUNING.itermRelax : 1
            );
            $('select[id="itermrelaxType"]').val(
                ADVANCED_TUNING.itermRelaxType
            );
            $('input[name="itermRelaxCutoff"]').val(
                ADVANCED_TUNING.itermRelaxCutoff
            );

            if (semver.gte(CONFIG.apiVersion, API_VERSION_1_43)) {
                $('.itermrelax input[name="itermRelaxCutoff"]').attr(
                    "max",
                    "50"
                );
            }
            ...
            // Absolute Control
            var absoluteControlGainNumberElement = $(
                'input[name="absoluteControlGain-number"]'
            );
            absoluteControlGainNumberElement
                .val(ADVANCED_TUNING.absoluteControlGain)
                .trigger("input");

            // Throttle Boost
            var throttleBoostNumberElement = $(
                'input[name="throttleBoost-number"]'
            );
            throttleBoostNumberElement
                .val(ADVANCED_TUNING.throttleBoost)
                .trigger("input");

            // Acro Trainer
            var acroTrainerAngleLimitNumberElement = $(
                'input[name="acroTrainerAngleLimit-number"]'
            );
            acroTrainerAngleLimitNumberElement
                .val(ADVANCED_TUNING.acroTrainerAngleLimit)
                .trigger("input");
            ...
            // Feedforward
            $('.pid_tuning .ROLL input[name="f"]').val(
                ADVANCED_TUNING.feedforwardRoll
            );
            $('.pid_tuning .PITCH input[name="f"]').val(
                ADVANCED_TUNING.feedforwardPitch
            );
            $('.pid_tuning .YAW input[name="f"]').val(
                ADVANCED_TUNING.feedforwardYaw
            );
            $("#pid_main .pid_titlebar2 th").attr("colspan", 5);

            var feedforwardTransitionNumberElement = $(
                'input[name="feedforwardTransition-number"]'
            );
            feedforwardTransitionNumberElement.val(
                ADVANCED_TUNING.feedforwardTransition / 100
            );

            // AntiGravity Mode
            var antiGravityModeSelect = $(
                '.antigravity select[id="antiGravityMode"]'
            );
        ...
        if (semver.gte(CONFIG.apiVersion, "1.41.0")) {
            $('select[id="throttleLimitType"]').val(
                RC_tuning.throttleLimitType
            );
            $('.throttle_limit input[name="throttleLimitPercent"]').val(
                RC_tuning.throttleLimitPercent
            );

            $('.pid_filter select[name="dtermLowpass2Type"]').val(
                FILTER_CONFIG.dterm_lowpass2_type
            );
            ...
            $('.pid_filter input[name="gyroLowpassDynMaxFrequency"]').val(
                FILTER_CONFIG.gyro_lowpass_dyn_max_hz
            );
            $('.pid_filter select[name="gyroLowpassDynType"]').val(
                FILTER_CONFIG.gyro_lowpass_type
            );
            $('.pid_filter input[name="dtermLowpassDynMinFrequency"]').val(
                FILTER_CONFIG.dterm_lowpass_dyn_min_hz
            );
            $('.pid_filter input[name="dtermLowpassDynMaxFrequency"]').val(
                FILTER_CONFIG.dterm_lowpass_dyn_max_hz
            );
            $('.pid_filter select[name="dtermLowpassDynType"]').val(
                FILTER_CONFIG.dterm_lowpass_type
            );

            $('.pid_tuning input[name="dMinRoll"]').val(
                ADVANCED_TUNING.dMinRoll
            );
            $('.pid_tuning input[name="dMinPitch"]').val(
                ADVANCED_TUNING.dMinPitch
            );
            $('.pid_tuning input[name="dMinYaw"]').val(ADVANCED_TUNING.dMinYaw);
            $('.dminGroup input[name="dMinGain"]').val(
                ADVANCED_TUNING.dMinGain
            );
            $('.dminGroup input[name="dMinAdvance"]').val(
                ADVANCED_TUNING.dMinAdvance
            );

            $('input[id="useIntegratedYaw"]').prop(
                "checked",
                ADVANCED_TUNING.useIntegratedYaw !== 0
            );
            //dmin column
        ...
        if (semver.gte(CONFIG.apiVersion, "1.42.0")) {
            ...
            $(".dynamicNotchRange").toggle(
                semver.lt(CONFIG.apiVersion, API_VERSION_1_43)
            );
            $('.pid_filter select[name="dynamicNotchRange"]').val(
                FILTER_CONFIG.dyn_notch_range
            );
            $('.pid_filter input[name="dynamicNotchWidthPercent"]').val(
                FILTER_CONFIG.dyn_notch_width_percent
            );
            $('.pid_filter input[name="dynamicNotchQ"]').val(
                FILTER_CONFIG.dyn_notch_q
            );
            $('.pid_filter input[name="dynamicNotchMinHz"]').val(
                FILTER_CONFIG.dyn_notch_min_hz
            );
            if (semver.gte(CONFIG.apiVersion, API_VERSION_1_43)) {
                $('.pid_filter input[name="dynamicNotchMinHz"]').attr(
                    "max",
                    "250"
                );
                $('.pid_filter input[name="dynamicNotchMaxHz"]').val(
                    FILTER_CONFIG.dyn_notch_max_hz
                );
            } else {
            ...
            $('.pid_filter input[name="rpmFilterHarmonics"]').val(
                FILTER_CONFIG.gyro_rpm_notch_harmonics
            );
            $('.pid_filter input[name="rpmFilterMinHz"]').val(
                FILTER_CONFIG.gyro_rpm_notch_min_hz
            );

            $(".pid_filter #rpmFilterEnabled")
                .change(function () {
                    let harmonics = $(
                        '.pid_filter input[name="rpmFilterHarmonics"]'
                    ).val();
                    let checked = $(this).is(":checked") && harmonics != 0;

                    $('.pid_filter input[name="rpmFilterHarmonics"]').attr(
                        "disabled",
                        !checked
                    );
                    $('.pid_filter input[name="rpmFilterMinHz"]').attr(
                        "disabled",
                        !checked
                    );
                    ...
                    if (harmonics == 0) {
                        $('.pid_filter input[name="rpmFilterHarmonics"]').val(
                            FILTER_DEFAULT.gyro_rpm_notch_harmonics
                        );
                    }
                })
                .prop("checked", FILTER_CONFIG.gyro_rpm_notch_harmonics != 0)
                .change();
        } else {
            $(".itermRelaxCutoff").hide();
            $(".dynamicNotch").hide();
        ...
        if (semver.gte(CONFIG.apiVersion, API_VERSION_1_43)) {
            $('.pid_tuning input[name="motorLimit"]').val(
                ADVANCED_TUNING.motorOutputLimit
            );
            $('.pid_tuning input[name="cellCount"]').val(
                ADVANCED_TUNING.autoProfileCellCount
            );
            $('input[name="idleMinRpm-number"]').val(
                ADVANCED_TUNING.idleMinRpm
            );
        } else {
        ...
        if (semver.gte(CONFIG.apiVersion, API_VERSION_1_43)) {
            ...
            for (let i = 0; i < ratesList.length; i++) {
                ratesTypeListElement.append(
                    `<option value="${i}">${ratesList[i].name}</option>`
                );
            }
            ...
            dMinSwitch.change(function () {
                ...
                if (checked) {
                    if (
                        ...
                        $('.pid_tuning input[name="dMinYaw"]').val(
                            Math.min(
                                Math.round(
                                    $(
                                        '.pid_tuning .YAW input[name="d"]'
                    ...
                    $("#pid_main .pid_titlebar2 th").attr("colspan", 5);
                    $(".derivativeText").text(
                        i18n.getMessage("pidTuningDerivative")
                    );
                }
            });
            dMinSwitch.change();
        ...
        $('input[id="gyroNotch1Enabled"]').change(function () {
            var checked = $(this).is(":checked");
            var hz =
                FILTER_CONFIG.gyro_notch_hz > 0
                    ? FILTER_CONFIG.gyro_notch_hz
                    : FILTER_DEFAULT.gyro_notch_hz;
            ...
            $('.pid_filter input[name="gyroNotch1Frequency"]')
                ...
                .change();
            $('.pid_filter input[name="gyroNotch1Cutoff"]')
                .attr("disabled", !checked)
                .change();
        });
        ...
        $('input[id="gyroNotch2Enabled"]').change(function () {
            var checked = $(this).is(":checked");
            var hz =
                FILTER_CONFIG.gyro_notch2_hz > 0
                    ? FILTER_CONFIG.gyro_notch2_hz
                    : FILTER_DEFAULT.gyro_notch2_hz;
            ...
            $('.pid_filter input[name="gyroNotch2Frequency"]')
                ...
                .attr("min", checked ? 1 : 0)
                .change();
            $('.pid_filter input[name="gyroNotch2Cutoff"]')
                .attr("disabled", !checked)
                .change();
        });
        ...
        $('input[id="dtermNotchEnabled"]').change(function () {
            var checked = $(this).is(":checked");
            var hz =
                FILTER_CONFIG.dterm_notch_hz > 0
                    ? FILTER_CONFIG.dterm_notch_hz
                    : FILTER_DEFAULT.dterm_notch_hz;
            ...
            $('.pid_filter input[name="dTermNotchFrequency"]')
                ...
                .attr("min", checked ? 1 : 0)
                .change();
            $('.pid_filter input[name="dTermNotchCutoff"]')
                .attr("disabled", !checked)
                .change();
        });
        ...
        $('input[id="gyroLowpassEnabled"]').change(function () {
            var checked = $(this).is(":checked");
            var disabledByDynamicLowpass = $(
                'input[id="gyroLowpassDynEnabled"]'
            ).is(":checked");
            ...
            $('.pid_filter input[name="gyroLowpassFrequency"]')
                .val(checked || disabledByDynamicLowpass ? cutoff : 0)
                .attr("disabled", !checked);
            $('.pid_filter select[name="gyroLowpassType"]')
                .val(type)
                .attr("disabled", !checked);
        ...
        $('input[id="gyroLowpassDynEnabled"]').change(function () {
            ...
            if (
                FILTER_CONFIG.gyro_lowpass_dyn_min_hz > 0 &&
                FILTER_CONFIG.gyro_lowpass_dyn_min_hz <
                    FILTER_CONFIG.gyro_lowpass_dyn_max_hz
            ) {
            ...
            $('.pid_filter input[name="gyroLowpassDynMinFrequency"]')
                .val(checked ? cutoff_min : 0)
                .attr("disabled", !checked);
            $('.pid_filter input[name="gyroLowpassDynMaxFrequency"]').attr(
                "disabled",
                !checked
            );
            $('.pid_filter select[name="gyroLowpassDynType"]')
                .val(type)
                .attr("disabled", !checked);
            ...
            if (checked) {
                ...
                FILTER_CONFIG.gyro_lowpass_hz > 0 &&
                !$('input[id="gyroLowpassEnabled"]').is(":checked")
            ) {
                $('input[id="gyroLowpassEnabled"]')
                    .prop("checked", true)
                    .change();
        ...
        $('input[id="gyroLowpass2Enabled"]').change(function () {
            var checked = $(this).is(":checked");
            var cutoff =
                FILTER_CONFIG.gyro_lowpass2_hz > 0
                    ? FILTER_CONFIG.gyro_lowpass2_hz
            ...
            $('.pid_filter input[name="gyroLowpass2Frequency"]')
                .val(checked ? cutoff : 0)
                .attr("disabled", !checked);
            $('.pid_filter select[name="gyroLowpass2Type"]')
                .val(type)
                .attr("disabled", !checked);
        });
        ...
        $('input[id="dtermLowpassEnabled"]').change(function () {
            var checked = $(this).is(":checked");
            var disabledByDynamicLowpass = $(
                'input[id="dtermLowpassDynEnabled"]'
            ).is(":checked");

            var cutoff =
                FILTER_CONFIG.dterm_lowpass_hz > 0
                    ? FILTER_CONFIG.dterm_lowpass_hz
                    : FILTER_DEFAULT.dterm_lowpass_hz;
            var type =
                FILTER_CONFIG.dterm_lowpass_hz > 0
                    ? FILTER_CONFIG.dterm_lowpass_type
                    : FILTER_DEFAULT.dterm_lowpass_type;

            $('.pid_filter input[name="dtermLowpassFrequency"]')
                .val(checked || disabledByDynamicLowpass ? cutoff : 0)
                .attr("disabled", !checked);
            $('.pid_filter select[name="dtermLowpassType"]')
                .val(type)
                .attr("disabled", !checked);
        ...
        $('input[id="dtermLowpassDynEnabled"]').change(function () {
            ...
            if (
                FILTER_CONFIG.dterm_lowpass_dyn_min_hz > 0 &&
                FILTER_CONFIG.dterm_lowpass_dyn_min_hz <
                    FILTER_CONFIG.dterm_lowpass_dyn_max_hz
            ) {
            ...
            $('.pid_filter input[name="dtermLowpassDynMinFrequency"]')
                .val(checked ? cutoff_min : 0)
                .attr("disabled", !checked);
            $('.pid_filter input[name="dtermLowpassDynMaxFrequency"]').attr(
                "disabled",
                !checked
            );
            $('.pid_filter select[name="dtermLowpassDynType"]')
                .val(type)
                .attr("disabled", !checked);

            if (checked) {
                $('input[id="dtermLowpassEnabled"]')
                    .prop("checked", false)
                    .change();
            } else if (
                FILTER_CONFIG.dterm_lowpass_hz > 0 &&
                !$('input[id="dtermLowpassEnabled"]').is(":checked")
            ) {
                $('input[id="dtermLowpassEnabled"]')
                    .prop("checked", true)
                    .change();
        ...
        $('input[id="dtermLowpass2Enabled"]').change(function () {
            var checked = $(this).is(":checked");
            var cutoff =
                FILTER_CONFIG.dterm_lowpass2_hz > 0
                    ? FILTER_CONFIG.dterm_lowpass2_hz
                    : FILTER_DEFAULT.dterm_lowpass2_hz;
            var type =
                FILTER_CONFIG.dterm_lowpass2_hz > 0
                    ? FILTER_CONFIG.dterm_lowpass2_type
                    : FILTER_DEFAULT.dterm_lowpass2_type;

            $('.pid_filter input[name="dtermLowpass2Frequency"]')
                .val(checked ? cutoff : 0)
                .attr("disabled", !checked);
            $('.pid_filter select[name="dtermLowpass2Type"]')
                .val(type)
                .attr("disabled", !checked);
        });
        ...
        function adjustNotchCutoff(frequencyName, cutoffName) {
            var frecuency = parseInt(
                $(".pid_filter input[name='" + frequencyName + "']").val()
            );
            var cutoff = parseInt(
                $(".pid_filter input[name='" + cutoffName + "']").val()
            );

            // Change the max and refresh the value if needed
            var maxCutoff = frecuency == 0 ? 0 : frecuency - 1;
            $(".pid_filter input[name='" + cutoffName + "']").attr(
                "max",
                maxCutoff
            );
            if (cutoff >= frecuency) {
        ...
        $('input[id="gyroNotch1Enabled"]')
            .prop("checked", FILTER_CONFIG.gyro_notch_hz != 0)
            .change();
        $('input[id="gyroNotch2Enabled"]')
            .prop("checked", FILTER_CONFIG.gyro_notch2_hz != 0)
            .change();
        $('input[id="dtermNotchEnabled"]')
            .prop("checked", FILTER_CONFIG.dterm_notch_hz != 0)
            .change();
        $('input[id="gyroLowpassEnabled"]')
        ...
        $('input[id="gyroLowpass2Enabled"]')
            .prop("checked", FILTER_CONFIG.gyro_lowpass2_hz != 0)
            .change();
        $('input[id="dtermLowpassEnabled"]')
            .prop("checked", FILTER_CONFIG.dterm_lowpass_hz != 0)
            .change();
        $('input[id="dtermLowpassDynEnabled"]')
            .prop(
                "checked",
            ...
            .change();
        $('input[id="dtermLowpass2Enabled"]')
            .prop("checked", FILTER_CONFIG.dterm_lowpass2_hz != 0)
            .change();
        $('input[id="yawLowpassEnabled"]')
            .prop("checked", FILTER_CONFIG.yaw_lowpass_hz != 0)
            .change();
    ...
    function form_to_pid_and_rc() {
        ...
        RC_tuning.roll_pitch_rate = parseFloat(
            $('.pid_tuning input[name="roll_pitch_rate"]').val()
        );
        ...
        RC_tuning.throttle_MID = parseFloat(
            $('.throttle input[name="mid"]').val()
        );
        RC_tuning.throttle_EXPO = parseFloat(
            $('.throttle input[name="expo"]').val()
        );

        RC_tuning.dynamic_THR_PID = parseFloat(
            $('.tpa input[name="tpa"]').val()
        );
        RC_tuning.dynamic_THR_breakpoint = parseInt(
            $('.tpa input[name="tpa-breakpoint"]').val()
        );
        FILTER_CONFIG.gyro_lowpass_hz = parseInt(
            $('.pid_filter input[name="gyroLowpassFrequency"]').val()
        );
        FILTER_CONFIG.dterm_lowpass_hz = parseInt(
            $('.pid_filter input[name="dtermLowpassFrequency"]').val()
        );
        FILTER_CONFIG.yaw_lowpass_hz = parseInt(
            $('.pid_filter input[name="yawLowpassFrequency"]').val()
        );
        ...
        if (semver.gte(CONFIG.apiVersion, "1.20.0")) {
            ADVANCED_TUNING.dtermSetpointTransition = parseInt(
                $('input[name="dtermSetpointTransition-number"]').val() * 100
            );
            ADVANCED_TUNING.dtermSetpointWeight = parseInt(
                $('input[name="dtermSetpoint-number"]').val() * 100
            );

            FILTER_CONFIG.gyro_notch_hz = parseInt(
                $('.pid_filter input[name="gyroNotch1Frequency"]').val()
            );
            FILTER_CONFIG.gyro_notch_cutoff = parseInt(
                $('.pid_filter input[name="gyroNotch1Cutoff"]').val()
            );
            FILTER_CONFIG.dterm_notch_hz = parseInt(
                $('.pid_filter input[name="dTermNotchFrequency"]').val()
            );
            FILTER_CONFIG.dterm_notch_cutoff = parseInt(
                $('.pid_filter input[name="dTermNotchCutoff"]').val()
            );
            if (semver.gte(CONFIG.apiVersion, "1.21.0")) {
                FILTER_CONFIG.gyro_notch2_hz = parseInt(
                    $('.pid_filter input[name="gyroNotch2Frequency"]').val()
                );
                FILTER_CONFIG.gyro_notch2_cutoff = parseInt(
                    $('.pid_filter input[name="gyroNotch2Cutoff"]').val()
                );
            }
        }
        ...
        if (semver.gte(CONFIG.apiVersion, "1.24.0")) {
            ADVANCED_TUNING.levelAngleLimit = parseInt(
                $('.pid_tuning input[name="angleLimit"]').val()
            );
            ADVANCED_TUNING.levelSensitivity = parseInt(
                $('.pid_tuning input[name="sensitivity"]').val()
            );
        }
        ...
        if (semver.gte(CONFIG.apiVersion, "1.36.0")) {
            ...
            ADVANCED_TUNING.itermThrottleThreshold = parseInt(
                $('.antigravity input[name="itermThrottleThreshold"]').val()
            );
            ADVANCED_TUNING.itermAcceleratorGain = parseInt(
                $('.antigravity input[name="itermAcceleratorGain"]').val() *
                    1000
            );
        }
        ...
        if (semver.gte(CONFIG.apiVersion, "1.39.0")) {
            FILTER_CONFIG.gyro_lowpass2_hz = parseInt(
                $('.pid_filter input[name="gyroLowpass2Frequency"]').val()
            );
            FILTER_CONFIG.gyro_lowpass_type = parseInt(
                $('.pid_filter select[name="gyroLowpassType"]').val()
            );
            FILTER_CONFIG.gyro_lowpass2_type = parseInt(
                $('.pid_filter select[name="gyroLowpass2Type"]').val()
            );
            FILTER_CONFIG.dterm_lowpass2_hz = parseInt(
                $('.pid_filter input[name="dtermLowpass2Frequency"]').val()
            );
        }

        if (semver.gte(CONFIG.apiVersion, "1.40.0")) {
            ADVANCED_TUNING.itermRotation = $('input[id="itermrotation"]').is(
                ":checked"
            )
                ? 1
                : 0;
            ADVANCED_TUNING.smartFeedforward = $(
                'input[id="smartfeedforward"]'
            ).is(":checked")
                ? 1
                : 0;

            ADVANCED_TUNING.itermRelax = $('input[id="itermrelax"]').is(
                ":checked"
            )
                ? $('select[id="itermrelaxAxes"]').val()
                : 0;
            ...
            ADVANCED_TUNING.itermRelaxCutoff = parseInt(
                $('input[name="itermRelaxCutoff"]').val()
            );

            ADVANCED_TUNING.absoluteControlGain = $(
                'input[name="absoluteControlGain-number"]'
            ).val();

            ADVANCED_TUNING.throttleBoost = $(
                'input[name="throttleBoost-number"]'
            ).val();

            ADVANCED_TUNING.acroTrainerAngleLimit = $(
                'input[name="acroTrainerAngleLimit-number"]'
            ).val();

            ADVANCED_TUNING.feedforwardRoll = parseInt(
                $('.pid_tuning .ROLL input[name="f"]').val()
            );
            ADVANCED_TUNING.feedforwardPitch = parseInt(
                $('.pid_tuning .PITCH input[name="f"]').val()
            );
            ADVANCED_TUNING.feedforwardYaw = parseInt(
                $('.pid_tuning .YAW input[name="f"]').val()
            );

            ADVANCED_TUNING.feedforwardTransition = parseInt(
                $('input[name="feedforwardTransition-number"]').val() * 100
            );

            ADVANCED_TUNING.antiGravityMode = $(
        ...
        if (semver.gte(CONFIG.apiVersion, "1.41.0")) {
            ...
            RC_tuning.throttleLimitPercent = parseInt(
                $('.throttle_limit input[name="throttleLimitPercent"]').val()
            );
            ...
            FILTER_CONFIG.gyro_lowpass_dyn_min_hz = parseInt(
                $('.pid_filter input[name="gyroLowpassDynMinFrequency"]').val()
            );
            FILTER_CONFIG.gyro_lowpass_dyn_max_hz = parseInt(
                $('.pid_filter input[name="gyroLowpassDynMaxFrequency"]').val()
            );
            FILTER_CONFIG.dterm_lowpass_dyn_min_hz = parseInt(
                $('.pid_filter input[name="dtermLowpassDynMinFrequency"]').val()
            );
            FILTER_CONFIG.dterm_lowpass_dyn_max_hz = parseInt(
                $('.pid_filter input[name="dtermLowpassDynMaxFrequency"]').val()
            );
            ...
            ADVANCED_TUNING.dMinRoll = parseInt(
                $('.pid_tuning input[name="dMinRoll"]').val()
            );
            ADVANCED_TUNING.dMinPitch = parseInt(
                $('.pid_tuning input[name="dMinPitch"]').val()
            );
            ADVANCED_TUNING.dMinYaw = parseInt(
                $('.pid_tuning input[name="dMinYaw"]').val()
            );
            ADVANCED_TUNING.dMinGain = parseInt(
                $('.dminGroup input[name="dMinGain"]').val()
            );
            ADVANCED_TUNING.dMinAdvance = parseInt(
                $('.dminGroup input[name="dMinAdvance"]').val()
            );

            ADVANCED_TUNING.useIntegratedYaw = $(
                'input[id="useIntegratedYaw"]'
            ).is(":checked")
                ? 1
                : 0;
        ...
        if (semver.gte(CONFIG.apiVersion, "1.42.0")) {
            FILTER_CONFIG.dyn_notch_range = parseInt(
                $('.pid_filter select[name="dynamicNotchRange"]').val()
            );
            FILTER_CONFIG.dyn_notch_width_percent = parseInt(
                $('.pid_filter input[name="dynamicNotchWidthPercent"]').val()
            );
            FILTER_CONFIG.dyn_notch_q = parseInt(
                $('.pid_filter input[name="dynamicNotchQ"]').val()
            );
            FILTER_CONFIG.dyn_notch_min_hz = parseInt(
                $('.pid_filter input[name="dynamicNotchMinHz"]').val()
            );

            let rpmFilterEnabled = $(".pid_filter #rpmFilterEnabled").is(
                ":checked"
            );
            FILTER_CONFIG.gyro_rpm_notch_harmonics = rpmFilterEnabled
                ? parseInt(
                      $('.pid_filter input[name="rpmFilterHarmonics"]').val()
                  )
                : 0;
            FILTER_CONFIG.gyro_rpm_notch_min_hz = parseInt(
                $('.pid_filter input[name="rpmFilterMinHz"]').val()
            );
        }

        if (semver.gte(CONFIG.apiVersion, API_VERSION_1_43)) {
            FILTER_CONFIG.dyn_notch_max_hz = parseInt(
                $('.pid_filter input[name="dynamicNotchMaxHz"]').val()
            );
            ADVANCED_TUNING.motorOutputLimit = parseInt(
                $('.pid_tuning input[name="motorLimit"]').val()
            );
            ADVANCED_TUNING.autoProfileCellCount = parseInt(
                $('.pid_tuning input[name="cellCount"]').val()
            );
            ADVANCED_TUNING.idleMinRpm = parseInt(
                $('input[name="idleMinRpm-number"]').val()
            );
            ...
            if (selectedRatesType !== RC_tuning.rates_type) {
                selectedRatesTypeName = $('select[id="ratesType"]')
    ...
    function hideUnusedPids() {
        ...
        isVisibleBaroMagGps |= hideSensorPid(
            $("#pid_baro"),
            have_sensor(CONFIG.activeSensors, "baro") ||
                have_sensor(CONFIG.activeSensors, "sonar")
        );

        isVisibleBaroMagGps |= hideSensorPid(
            $("#pid_mag"),
            have_sensor(CONFIG.activeSensors, "mag")
        );

        isVisibleBaroMagGps |= hideSensorPid(
            $("#pid_gps"),
            have_sensor(CONFIG.activeSensors, "GPS")
        );
    ...
    function checkInput(element) {
        ...
        if (
            value < parseFloat(element.prop("min")) ||
    ...
    function printMaxAngularVel(
        ...
        deadband,
        limit,
        maxAngularVelElement
    ) {
        var maxAngularVel = self.rateCurve
            .getMaxAngularVel(
                rate,
                rcRate,
                rcExpo,
    ...
    function process_html() {
        FEATURE_CONFIG.features.generateElements(
            $(".tab-pid_tuning .features")
        );

        if (
            semver.lt(CONFIG.apiVersion, "1.16.0") ||
            semver.gte(CONFIG.apiVersion, "1.20.0")
        ) {
        ...
        if (semver.lt(CONFIG.apiVersion, "1.39.0")) {
            $('input[name="dtermSetpoint-number"]').attr(
                "max",
                self.SETPOINT_WEIGHT_RANGE_LEGACY
            );
        }
        ...
        $(".tab-pid_tuning .tab-container .pid").on("click", () =>
            activateSubtab("pid")
        );

        $(".tab-pid_tuning .tab-container .rates").on("click", () =>
            activateSubtab("rates")
        );

        $(".tab-pid_tuning .tab-container .filter").on("click", () =>
        ...
        function loadProfilesList() {
            ...
            for (var i = 0; i < numberOfProfiles; i++) {
                profileElements.push(
                    i18n.getMessage("pidTuningProfileOption", [i + 1])
                );
            }
        ...
        function loadRateProfilesList() {
            ...
            for (var i = 0; i < numberOfRateProfiles; i++) {
                rateProfileElements.push(
                    i18n.getMessage("pidTuningRateProfileOption", [i + 1])
                );
            }
        ...
        $('.tab-pid_tuning select[name="profile"]').change(function () {
            ...
            MSP.promise(MSPCodes.MSP_SELECT_SETTING, [
                self.currentProfile,
            ]).then(function () {
                self.refresh(function () {
                    self.updating = false;

                    $('.tab-pid_tuning select[name="profile"]').prop(
                        "disabled",
                        "false"
                    );
                    ...
                    GUI.log(
                        i18n.getMessage("pidTuningLoadedProfile", [
        ...
        if (semver.gte(CONFIG.apiVersion, "1.20.0")) {
            $('.tab-pid_tuning select[name="rate_profile"]').change(
                function () {
                    ...
                    $(this).prop("disabled", "true");
                    MSP.promise(MSPCodes.MSP_SELECT_SETTING, [
                        self.currentRateProfile + self.RATE_PROFILE_MASK,
                    ]).then(function () {
                        self.refresh(function () {
                            self.updating = false;

                            $(
                                '.tab-pid_tuning select[name="rate_profile"]'
                            ).prop("disabled", "false");
                            CONFIG.rateProfile = self.currentRateProfile;

                            GUI.log(
                                i18n.getMessage("pidTuningLoadedRateProfile", [
                                    self.currentRateProfile + 1,
                                ])
                            );
                        });
                    });
            ...
            var dtermTransitionNumberElement = $(
                'input[name="dtermSetpointTransition-number"]'
            );
            var dtermTransitionWarningElement = $(
                "#pid-tuning .dtermSetpointTransitionWarning"
            );
            ...
            checkUpdateDtermTransitionWarning(
                dtermTransitionNumberElement.val()
            );
        ...
        function loadDynamicNotchRangeValues() {
            var dynamicNotchRangeValues = ["HIGH", "MEDIUM", "LOW", "AUTO"];
            return dynamicNotchRangeValues;
        }
        function populateDynamicNotchRangeSelect(
            selectDynamicNotchRangeValues
        ) {
            var dynamicNotchRangeSelect = $('select[name="dynamicNotchRange"]');
            selectDynamicNotchRangeValues.forEach(function (value, key) {
                dynamicNotchRangeSelect.append(
                    '<option value="' + key + '">' + value + "</option>"
                );
            });
        }
        ...
        function activateSubtab(subtabName) {
            ...
            $(".tab-pid_tuning .tab-container ." + subtabName).addClass(
                "active"
            );
        ...
        if (semver.gte(CONFIG.apiVersion, "1.37.0")) {
            ...
            $(".pid_tuning input[name=rc_rate]")
                .parent()
                .attr("class", "pid_data");
            $(".pid_tuning input[name=rc_rate]").parent().attr("rowspan", 1);
            $(".pid_tuning input[name=rc_expo]")
                .parent()
                .attr("class", "pid_data");
        ...
        function updateRates(event) {
            setTimeout(function () {
                ...
                if (event) {
                    ...
                    if (
                        self.currentRates.hasOwnProperty(
                            targetElement.attr("name")
                        ) &&
                        targetValue !== undefined
                    ) {
                        const stepValue = parseFloat(
                            targetElement.prop("step")
                        ); // adjust value to match step (change only the result, not the the actual value)
                        if (stepValue != null) {
                            targetValue =
                    ...
                    if (
                        targetElement.attr("name") === "rc_rate" &&
                        semver.lt(CONFIG.apiVersion, "1.16.0")
                    ) {
                    ...
                    if (
                        targetElement.attr("name") === "roll_pitch_rate" &&
                        semver.lt(CONFIG.apiVersion, "1.7.0")
                    ) {
                    ...
                    if (targetElement.attr("name") === "SUPEREXPO_RATES") {
                        self.currentRates.superexpo = targetElement.is(
                            ":checked"
                        );
                    ...
                    if (
                        targetElement.attr("name") === "rc_rate" &&
                        semver.lt(CONFIG.apiVersion, "1.37.0")
                    ) {
                        self.currentRates.rc_rate_pitch = targetValue;
                    }

                    if (
                        targetElement.attr("name") === "rc_expo" &&
                        semver.lt(CONFIG.apiVersion, "1.37.0")
                    ) {
                    ...
                    if (
                        targetElement.attr("id") === "ratesType" &&
                        semver.gte(CONFIG.apiVersion, API_VERSION_1_43)
                    ) {
                ...
                if (updateNeeded) {
                    ...
                    var lineScale =
                        curveContext.canvas.width /
                        curveContext.canvas.clientWidth;
                    ...
                    if (!useLegacyCurve) {
                        ...
                        maxAngularVel = self.rateCurve.setMaxAngularVel(
                            maxAngularVel
                        );
        ...
        TABS.pid_tuning.throttleDrawInterval = setInterval(
            redrawThrottleCurve,
            100
        );
        ...
        $("#pid-tuning")
            ...
            .each(function (k, item) {
                if (
                    $(item).attr("class") !== "feature toggle" &&
            ...
            $(".dialogCopyProfile-confirmbtn").click(function () {
                switch (dialogCopyProfileMode) {
                    ...
                    case DIALOG_MODE_RATEPROFILE:
                        ...
                        COPY_PROFILE.dstProfile = parseInt(
                            selectRateProfile.val()
                        );
        ...
        if (semver.gte(CONFIG.apiVersion, "1.42.0")) {
            ...
            // pid sliders inputs
            $(
                "#tuningMasterSlider, #tuningPDRatioSlider, #tuningPDGainSlider, #tuningResponseSlider"
            ).on("input", function () {
                const slider = $(this);
                ...
                const scaledValue = TuningSliders.scaleSliderValue(
                    slider.val()
                );
            ...
            $("#tuningGyroFilterSlider, #tuningDTermFilterSlider").on(
                "input",
                function () {
                    const slider = $(this);
                    if (!TuningSliders.expertMode) {
            ...
            $(
                ".pid_filter tr:not(.newFilter) input, .pid_filter tr:not(.newFilter) select"
            ).on("input", function () {
                TuningSliders.updateFilterSlidersDisplay();
                if (TuningSliders.filterGyroSliderUnavailable) {
        ...
        $("a.update").click(function () {
            ...
            Promise.resolve(true)
                .then(function () {
                    ...
                    if (
                        ...
                        semver.lt(CONFIG.apiVersion, "1.31.0")
                    ) {
                        PID.controller = pidController_e.val();
                        promise = MSP.promise(
                            MSPCodes.MSP_SET_PID_CONTROLLER,
                            mspHelper.crunch(MSPCodes.MSP_SET_PID_CONTROLLER)
                        );
                    }
                    return promise;
                })
                .then(function () {
                    return MSP.promise(
                        ...
                        mspHelper.crunch(MSPCodes.MSP_SET_PID)
                    );
                })
                .then(function () {
                    return MSP.promise(
            ...
            analytics.sendChangeEvents(
                analytics.EVENT_CATEGORIES.FLIGHT_CONTROLLER,
...
TABS.pid_tuning.initRatesPreview = function () {
    ...
    $(".tab-pid_tuning .tab-container .rates").on(
        "click",
        $.proxy(this.model.resize, this.model)
    );
    $(".tab-pid_tuning .tab-container .rates").on(
        "click",
        $.proxy(this.updateRatesLabels, this)
    );
...
TABS.pid_tuning.setRateProfile = function () {
    ...
    $('.tab-pid_tuning select[name="rate_profile"]').val(
...
TABS.pid_tuning.setDirty = function (isDirty) {
    ...
    if (semver.gte(CONFIG.apiVersion, "1.20.0")) {
        $('.tab-pid_tuning select[name="rate_profile"]').prop(
            "disabled",
            isDirty
        );
    }
        ...
        if (!self.updating && !self.dirty) {
            ...
            if (changedProfile || changedRateProfile) {
                self.refresh(function () {
                    if (changedProfile) {
                        GUI.log(
                            i18n.getMessage("pidTuningReceivedProfile", [
                                CONFIG.profile + 1,
                            ])
                        );
                        CONFIG.profile = self.currentProfile;
                    ...
                    if (changedRateProfile) {
                        GUI.log(
                            i18n.getMessage("pidTuningReceivedRateProfile", [
                                CONFIG.rateProfile + 1,
                            ])
                        );
                        CONFIG.rateProfile = self.currentRateProfile;
                    }
...
TABS.pid_tuning.updatePidControllerParameters = function () {
    if (
        semver.gte(CONFIG.apiVersion, "1.20.0") &&
        semver.lt(CONFIG.apiVersion, "1.31.0") &&
        $('.tab-pid_tuning select[name="controller"]').val() === "0"
    ) {
    ...
    if (!self.rateCurve.useLegacyCurve && self.rateCurve.maxAngularVel) {
        ...
        var drawBalloonLabel = function (
            ...
            for (var i = 0; i < dirty.length; i++) {
                if (
                    ...
                    if (
                        ...
                        (y + height >= dirty[i].top &&
                            y + height <= dirty[i].bottom)
                    ) {
            ...
            if (align == "right") {
                ...
                context.lineTo(
                    x + width,
                    y + height - DEFAULT_RADIUS - pointerLength
                );
            }
        ...
        if (rcStickElement) {
            ...
            var maxAngularVelRoll =
                    self.maxAngularVelRollElement.text() + " deg/s",
                maxAngularVelPitch =
                    self.maxAngularVelPitchElement.text() + " deg/s",
                maxAngularVelYaw =
                    self.maxAngularVelYawElement.text() + " deg/s",
                currentValues = [],
                balloonsDirty = [],
                ...
                lineScale =
                    stickContext.canvas.width / stickContext.canvas.clientWidth,
                textScale =
                    stickContext.canvas.clientHeight /
                    stickContext.canvas.clientWidth;
            ...
            if (windowScale <= 1) {
                ...
                stickContext.font =
                    24 * windowScale + "pt Verdana, Arial, sans-serif";
            }
            ...
            var balloons = [
                {
                    ...
                    balloon: function () {
                        drawBalloonLabel(
                            ...
                            curveWidth,
                            rateScale *
                                (maxAngularVel - parseInt(maxAngularVelRoll)),
                            "right",
                            BALLOON_COLORS.roll,
                            balloonsDirty
                        );
                ...
                },
                {
                    ...
                    balloon: function () {
                        drawBalloonLabel(
                            ...
                            curveWidth,
                            rateScale *
                                (maxAngularVel - parseInt(maxAngularVelPitch)),
                            "right",
                ...
                },
                {
                    ...
                    balloon: function () {
                        drawBalloonLabel(
                            ...
                            curveWidth,
                            rateScale *
                                (maxAngularVel - parseInt(maxAngularVelYaw)),
                            "right",
                            BALLOON_COLORS.yaw,
                            balloonsDirty
                        );
...
TABS.pid_tuning.updateFilterWarning = function () {
    var gyroDynamicLowpassEnabled = $('input[id="gyroLowpassDynEnabled"]').is(
        ":checked"
    );
    var gyroLowpass1Enabled = $('input[id="gyroLowpassEnabled"]').is(
        ":checked"
    );
    var dtermDynamicLowpassEnabled = $('input[id="dtermLowpassDynEnabled"]').is(
        ":checked"
    );
    var dtermLowpass1Enabled = $('input[id="dtermLowpassEnabled"]').is(
        ":checked"
    );
    var warning_e = $("#pid-tuning .filterWarning");
    var warningDynamicNotch_e = $("#pid-tuning .dynamicNotchWarning");
    if (
        !(gyroDynamicLowpassEnabled || gyroLowpass1Enabled) ||
        !(dtermDynamicLowpassEnabled || dtermLowpass1Enabled)
    ) {
        warning_e.show();
    } else {
        warning_e.hide();
...
TABS.pid_tuning.updatePIDColors = function (clear = false) {
    const setTuningElementColor = function (element, mspValue, currentValue) {
        ...
        element.css({
            "background-color": cssUtil.getColorForPercentage(
                change,
                cssUtil.colorTables.pidSlider
            ),
        });
    };
    ...
    PID_names.forEach(function (elementPid, indexPid) {
        $(".pid_tuning ." + elementPid + " input").each(function (indexInput) {
            setTuningElementColor(
                $(this),
                PIDS_ACTIVE[indexPid][indexInput],
    ...
    setTuningElementColor(
        $('.pid_tuning input[name="dMinRoll"]'),
        ADVANCED_TUNING_ACTIVE.dMinRoll,
        ADVANCED_TUNING.dMinRoll
    );
    setTuningElementColor(
        $('.pid_tuning input[name="dMinPitch"]'),
        ADVANCED_TUNING_ACTIVE.dMinPitch,
        ADVANCED_TUNING.dMinPitch
    );
    setTuningElementColor(
        $('.pid_tuning input[name="dMinYaw"]'),
        ADVANCED_TUNING_ACTIVE.dMinYaw,
        ADVANCED_TUNING.dMinYaw
    );
    setTuningElementColor(
        $('.pid_tuning .ROLL input[name="f"]'),
        ADVANCED_TUNING_ACTIVE.feedforwardRoll,
        ADVANCED_TUNING.feedforwardRoll
    );
    setTuningElementColor(
        $('.pid_tuning .PITCH input[name="f"]'),
        ADVANCED_TUNING_ACTIVE.feedforwardPitch,
        ADVANCED_TUNING.feedforwardPitch
    );
    setTuningElementColor(
        $('.pid_tuning .YAW input[name="f"]'),
        ADVANCED_TUNING_ACTIVE.feedforwardYaw,
        ADVANCED_TUNING.feedforwardYaw
    );
};
...
TABS.pid_tuning.changeRatesSystem = function (sameType) {
    ...
    rc_rate_input_c
        .attr({ max: rcRateMax, min: rcRateMin, step: rcRateStep })
        .change();
